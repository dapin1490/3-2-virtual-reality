...
public class LectureJump : MonoBehaviour
{
    ...
    {
        isGrounded = true;
        this.animator.SetBool("isGround", true); //애니메이터 파일 변수에서 땅에 있는 상황을 알려 줍니다.
    }

    void Update()
    {
        if (Input.GetButtonDown("Fire1") && readyattack) // &&는 양측 조건이 모두 만족될 때 아래 내용을 실행합니다. readyattack==true의미입니다.
        {
            readyattack = false; //공격 후 바로 공격이 안되도록 합니다.
            this.animator.SetTrigger("attack"); //공격 애니메이션 트리거 신호를 보냅니다.
        ...
        if (isGrounded)
        {
            ...
            {
                this.animator.SetTrigger("jump"); //점프 애니메이션을 실행합니다. 
                this.animator.SetBool("isGround", false); //공중에 있는 조건으로 바꿉니다.
